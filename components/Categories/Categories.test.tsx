import { mount } from '@cypress/react';

import { withThemeWrapper } from 'tools/testing';
import CategoriesList from './CategoriesList';
import CategoriesItem, { DEFAULT_IMG_WIDTH } from './CategoriesItem';

const testImage =
  'data:image/gif;base64,';

describe('CategoriesItem', () => {
  it('Default render (with required props)', () => {
    mount(
      withThemeWrapper(
        <CategoriesItem data-cy="category-item" href="www.test.com">
          Test content
        </CategoriesItem>,
      ),
    );

    cy.get('*[data-cy=category-item]').invoke('prop', 'tagName').should('eq', 'A');
    cy.get('*[data-cy=category-item]').invoke('attr', 'href').should('eq', '/www.test.com');
    cy.get('*[data-cy=category-item]').children().should('have.length', 1);
    cy.get('*[data-cy=category-item]').children().first().should('contain', 'Test content');
  });

  it('Render with title', () => {
    mount(
      withThemeWrapper(
        <CategoriesItem data-cy="category-item" href="www.test.com" title="I's test title">
          Test content
        </CategoriesItem>,
      ),
    );
    cy.get('*[data-cy=category-item]').invoke('prop', 'tagName').should('eq', 'A');
    cy.get('*[data-cy=category-item]').invoke('attr', 'href').should('eq', '/www.test.com');
    cy.get('*[data-cy=category-item]').children().should('have.length', 2);
    cy.get('*[data-cy=category-item]').children().eq(0).should('contain', "I's test title");
    cy.get('*[data-cy=category-item]').children().eq(1).should('contain', 'Test content');
  });

  it('Render with title and image', () => {
    mount(
      withThemeWrapper(
        <CategoriesItem
          data-cy="category-item"
          href="www.test.com"
          title="I's test title"
          imageSrc={testImage}
        >
          Test content
        </CategoriesItem>,
      ),
    );

    cy.get('*[data-cy=category-item]').invoke('attr', 'href').should('eq', '/www.test.com');
    cy.get('*[data-cy=category-item]').children().should('have.length', 2);
    cy.get('*[data-cy=category-item]').children().eq(0).should('contain', "I's test title");
    cy.get('*[data-cy=category-item]')
      .children()
      .eq(0)
      .find('img')
      .invoke('attr', 'src')
      .should('eq', testImage);
    cy.get('*[data-cy=category-item]').children().eq(1).should('contain', 'Test content');
  });

  it('Default image size', () => {
    mount(
      withThemeWrapper(
        <CategoriesItem
          data-cy="category-item"
          href="www.test.com"
          title="I's test title"
          imageSrc={testImage}
        >
          Test content
        </CategoriesItem>,
      ),
    );

    cy.get('*[data-cy=category-item]')
      .children()
      .eq(0)
      .find('img')
      .parent()
      .parent()
      .should('have.css', 'max-width', `${DEFAULT_IMG_WIDTH}px`);
  });

  it('Custom image size', () => {
    mount(
      withThemeWrapper(
        <CategoriesItem
          data-cy="category-item"
          href="www.test.com"
          title="I's test title"
          imageSrc={testImage}
          imageMaxWidth={66}
        >
          Test content
        </CategoriesItem>,
      ),
    );

    cy.get('*[data-cy=category-item]')
      .children()
      .eq(0)
      .find('img')
      .parent()
      .parent()
      .should('have.css', 'max-width', `${66}px`);
  });
});

describe('CategoriesList', () => {
  it('Children', () => {
    mount(
      withThemeWrapper(
        <CategoriesList data-cy="categories-list">
          <div>child 1</div>
          <div>child 2</div>
        </CategoriesList>,
      ),
    );

    cy.get('*[data-cy=categories-list]').children().should('have.length', 2);
    cy.get('*[data-cy=categories-list]').children().eq(0).should('contain', 'child 1');
    cy.get('*[data-cy=categories-list]').children().eq(1).should('contain', 'child 2');
  });

  it('Children size', () => {
    mount(
      withThemeWrapper(
        <CategoriesList data-cy="categories-list" maxCardSize={100}>
          <div>child 1</div>
          <div>child 2</div>
        </CategoriesList>,
      ),
    );

    cy.get('*[data-cy=categories-list]').children().first().should('have.css', 'width', '103px');
  });
});
