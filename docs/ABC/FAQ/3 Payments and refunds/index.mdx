...
Payment statuses (also called payment actions) refer to the various statuses that a given payment goes through. For example, a card payment might go from being authorized (AUTH) to captured (CAPTURE) before finally being refunded (REFUND). Each of these statuses will be displayed in the **Action** column of the [Hub](https://hub.checkout.com/login)'s Payments screen, as well as in the payment details.
...
You can issue a refund (either full or partial) through the [Hub](https://hub.checkout.com/login) by [following these steps](https://docs.ckotech.co/the-hub/manage-payments#Managepayments-Capture,void,orrefundapayment), or by calling the [refund endpoint](/payments/manage-payments/refund-a-payment).
...
- using the [Hub](https://hub.checkout.com/login). To do so, simply open the customer details page and click the **Create payment** button.
...
You can find the Acquirer Reference Number (ARN) for a payment in the _Payments_ section of the [Hub](https://hub.checkout.com/login). Just click on the relevant payment action to open up the summary, and you'll find the ARN under the **Action details** heading.
...
To do so, go to the _Customers_ section of your [Hub](https://hub.checkout.com/login) account, select the customer you want to charge and click **Create payment**.
...
No. To process a refund, the original transaction must have been be processed with us and be visible in your [Hub](https://hub.checkout.com/login) account.
