...
Because the `frames-react` components are iframes, there are specific ways to interact with them. That’s why we use the `Frames` component’s several built-in props to handle Frames custom events. <ExternalLink href="https://github.com/checkout/frames-react#the-props" target="_blank">Read more about available props</ExternalLink>.
