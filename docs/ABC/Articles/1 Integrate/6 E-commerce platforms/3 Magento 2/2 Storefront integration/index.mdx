...
To get started, follow Magento's <ExternalLink href="https://devdocs.magento.com/guides/v2.3/graphql/tutorials/checkout/" target="_blank">checkout tutorial</ExternalLink> until you get to step 9. Then, instead of following steps 9 and 10, use our request payment endpoint below to complete the order.
...
See <ExternalLink href="https://devdocs.magento.com/guides/v2.4/rest/tutorials/orders/order-create-customer.html#get-token" target="_blank">Magento documentation</ExternalLink> to generate a customer token.
...
Use the valid public key of your <ExternalLink href="https://hub.checkout.com" target="_blank">Checkout.com](http://checkout.com/) account. You can find this in the [Hub</ExternalLink>.
...
If the payment was made with [3D Secure (3DS) authentication](/risk-management/3-d-secure), you will get a `200` response containing a redirect link that the customer will need to complete in order to finalize the transaction.
...
Once 3DS authentication is completed, you will receive a `session_id` from our payment gateway. You can pass this `session_id` to our [get payment details endpoint](/risk-management/3-d-secure/3-d-secure-2-api-integration) to determine whether the payment was approved, or to get more information about it.
...
Once 3DS authentication is completed, you will receive a `session_id` from our payment gateway. You can pass this `session_id` to our [get payment details endpoint](/risk-management/3-d-secure/3d-secure-2-0-api-integration#id-3DSecure2.0APIintegration-Step4:VerifythepaymentwiththesessionIDverifypayment) to determine whether the payment was approved, or to get more information about it.
