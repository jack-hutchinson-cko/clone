...
4.  In the Cartridges field, add one of the following to the front of the cartridge path and select _Apply+:
...
5.  Under Upload Archive, select _Local_ and then _Choose file_.
...
5.  Set Mode to _Sandbox_, so you can start testing card payments.
6.  Enter your business name (the name of your shop), and fill in the business address, city and country fields.
7.  Choose whether or not you want to Auto capture your payment. Set to _Yes_ if you want your payment to be captured (i.e., the money due from the customer's account is moved into your account) immediately after being authorized. Set to _No_ if you want to capture the payment manually.
8.  If you set Auto capture to _Yes_, you can specify the delay (in hours) before the payment is captured. For example, entering `1.5` would capture the payment one hour and 30 minutes after authorization. The default is `0`.
9.  Select _Save_.
