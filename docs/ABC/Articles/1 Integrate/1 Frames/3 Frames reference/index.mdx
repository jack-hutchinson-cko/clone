...
When [paying out to a card](/card-payouts), the [CVV field](/risk-management/cvv-check) is not mandatory, so you can initialize Frames with `CVV_HIDDEN` **or** `CVV_OPTIONAL`.
