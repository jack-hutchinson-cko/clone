...
Because Latin American card numbers vary in length and structure, they cannot be validated with a <ExternalLink href="https://en.wikipedia.org/wiki/Luhn_algorithm" target="_blank">Luhn check</ExternalLink>. You should disable this check when making a `dlocal` payment request to avoid the card number being flagged as invalid to the customer and to prevent the request from being rejected.
