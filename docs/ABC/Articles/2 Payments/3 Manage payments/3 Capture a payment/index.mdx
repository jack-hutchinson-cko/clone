...
If a payment is created with `"capture": "false"`, you can either use this endpoint to capture the payment or capture it from the <ExternalLink href="https://hub.checkout.com/v2" target="_blank">Hub</ExternalLink>. Manual capture is not allowed if `capture` is set to `true`.
