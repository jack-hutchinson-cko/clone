...
In the Params tab, enter `From` in the _Key_ column. Add the start date of your timeframe in the _Value_ column — this must be in ISO format (`YYYY-MM-DD`). In the row below, enter To in the _Key_ column and the end date of your timeframe in the _Value_ column.
...
In the Headers tab, enter `Authorization` in the _Key_ column. In the _Value_ column, add your secret key. You can find your secret key in the _Settings > Channels_ section of the [Hub](https://hub.checkout.com/). Next, enter `Content-Type` in the _Key_ column and `application/json` in the _Value_ column.
...
In the _Headers_ tab, enter `Authorization` in the _Key_ column. In the _Value_ column, add your secret key. You can find your secret key in the _Settings > Channels_ section of the [Hub](https://hub.checkout.com/login).
