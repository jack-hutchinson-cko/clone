...
Frames is an iframe solution that handles payment information, so you remain PCI-compliant. Your customer inputs their card details directly into our iframes, then we send you a token representing those details, so you can request a payment.
