...
Start accepting payments using <ExternalLink href="https://www.checkout.com/payment-methods/accept/ideal" target="_blank">iDEAL</ExternalLink>, a favorite online payment method in The Netherlands.
...
Use the details below to set up your request. To get a detailed view of all required and optional fields, see our <ExternalLink href="https://api-reference.checkout.com/preview/crusoe" target="_blank">API reference</ExternalLink>.
...
For security reasons, iDEAL forbids using custom-made in-app browsers for redirection. We recommend that you open the device’s default browser to display the redirect. If you do want to use an in-app web view, you must use <ExternalLink href="https://developer.chrome.com/multidevice/android/customtabs" target="_blank">SFSafariViewController](https://developer.apple.com/documentation/safariservices/sfsafariviewcontroller) for iOS or [Chrome Custom Tabs</ExternalLink> for Android.
...
You can use the `payment_id` found in the payment response, or the `cko-session-id` from the success/failure URL (e.g., `<ExternalLink href="https://www.checkout.com/order/succeeded?cko-session-id=)**sid_vii64oquze5u3h2x6hh4rurc4y**`" target="_blank">https://www.checkout.com/order/succeeded?cko-session-id=</ExternalLink> to retrieve details about the payment. To get a detailed view of all required and optional fields, see our <ExternalLink href="https://api-reference.checkout.com/preview/crusoe" target="_blank">API reference</ExternalLink>.
...
iDEAL supports both partial and full refunds. You can refund a payment through the <ExternalLink href="https://api-reference.checkout.com/#operation/refundAPayment" target="_blank">Hub](https://hub.checkout.com/v2) or using the [refund endpoint</ExternalLink> of the Unified Payments API.
...
Use the following request to get an up-to-date list of all issuers supporting iDEAL payments. To get a detailed view of all required and optional fields, see our <ExternalLink href="https://api-reference.checkout.com/preview/crusoe" target="_blank">API reference</ExternalLink>.
...
*   <ExternalLink href="https://www.checkout.com/get-test-account" target="_blank">create a test account</ExternalLink>, and
