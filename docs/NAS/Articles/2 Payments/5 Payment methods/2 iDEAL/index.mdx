...
You can use the `payment_id` found in the payment response, or the `cko-session-id` from the success/failure URL (e.g., `[https://www.checkout.com/order/succeeded?cko-session-id=](https://www.checkout.com/order/succeeded?cko-session-id=)**sid_vii64oquze5u3h2x6hh4rurc4y**`) to retrieve details about the payment. To get a detailed view of all required and optional fields, see our <ExternalLink href="https://api-reference.checkout.com/preview/crusoe" target="_blank">API reference</ExternalLink>.
