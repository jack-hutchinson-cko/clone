...
You can find the full list, as well as complete request and response examples, in our [API reference](https://api-reference.checkout.com/preview/crusoe).
...
Fawry supports both partial (see note below for details) and full refunds. You can refund a payment through the [Dashboard](https://dashboard.checkout.com/login) or by using the [refund API](/payments/manage-payments/refund-a-payment).
