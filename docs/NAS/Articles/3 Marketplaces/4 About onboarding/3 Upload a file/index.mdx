---
title: Upload a file 
account: NAS
---

Use our Files API to upload identity documentation required for `full` due diligence as part of our Marketplace solution.

When you upload a file, the API returns an `id` that represents that file. You can use this `id` to attach identity documentation when [creating or updating your sub-entity.](/marketplaces/about-onboarding/onboard-a-sub-entity-full)

---

## Types of identification and requirements

The API accepts the following types of documentation. Depending on the identification type, you may need to provide both the back and the front. Use the table below to find out more.

<Table>
<TableHead headers={['Type','Front required','Back required']}></TableHead>
<TableBody>
<TableRow>
<TableCell>

Passport

</TableCell>
<TableCell>

Y

</TableCell>
<TableCell>

N

</TableCell>
</TableRow>
<TableRow>
<TableCell>

Driving licence

</TableCell>
<TableCell>

Y

</TableCell>
<TableCell>

Y

</TableCell>
</TableRow>
<TableRow>
<TableCell>

Driving licence

</TableCell>
<TableCell>

Y

</TableCell>
<TableCell>

Y

</TableCell>
</TableRow>
<TableRow>
<TableCell>

National identity card

</TableCell>
<TableCell>

Y

</TableCell>
<TableCell>

Y

</TableCell>
</TableRow>
<TableRow>
<TableCell>

Citizen card

</TableCell>
<TableCell>

Y

</TableCell>
<TableCell>

Y

</TableCell>
</TableRow>
<TableRow>
<TableCell>

Residence permit

</TableCell>
<TableCell>

Y

</TableCell>
<TableCell>

Y

</TableCell>
</TableRow>
<TableRow>
<TableCell>

Electoral ID

</TableCell>
<TableCell>

Y

</TableCell>
<TableCell>

Y

</TableCell>
</TableRow>
</TableBody>
</Table>

---

## Upload a file

Your request must be multipart/form-data and the file must be in either JPEG/JPG or PNG format, and no larger than 4MB.

Follow the steps to upload the document and receive an `id`. If the identification you are providing requires both the back and front, you will need to make **two requests** to the API.

### Endpoints

Sandbox is a testing area, so you can practise API calls. [Request a test account.](https://www.checkout.com/get-test-account)

<TipBox variant='note'>

Notice that the sub-domain – files.checkout.com – is slightly different to Checkout.com's other endpoints.

</TipBox>

<tabs>
<tab title='Live'>
<LineCodeWrapper type="post">

`https://files.checkout.com`

</LineCodeWrapper>
</tab>
<tab title='Sandbox'>
<LineCodeWrapper type="post">

`https://files.sandbox.checkout.com`

</LineCodeWrapper>
</tab>
</tabs>

### Request example

Your request should include the **type** of document, the **file path** and the **purpose** of the document. When uploading an identification document, set this to `identification`. For a full list of possible fields, see our [API reference.](https://api-reference.checkout.com/preview/crusoe/#tag/Marketplace/)


<Table>
<TableHead headers={['Header','Description']}></TableHead>
<TableBody>
<TableRow>
<TableCell>

Authorization
<StatusTag type="required" />

</TableCell>
<TableCell>

`OAuth Bearer access token`

</TableCell>
</TableRow>
<TableRow>
<TableCell>

Content-Type
<StatusTag type="required" />

</TableCell>
<TableCell>

`multipart/form-data`

</TableCell>
</TableRow>
</TableBody>
</Table>

```json
curl https://files.checkout.com \
  -F "type=passport" \
  -F "cw_passport.jpg" \
  -F "purpose=identification"
```

### Response examples

<tabs>
<tab title='201 - Document accepted'>
<Box horizontal='sm'>

If you received a file `id` in your response, the document was accepted. We will automatically make the association between this document and its representative. You can see this association by [retrieving the sub-entity.](/marketplaces/about-onboarding/onboard-a-sub-entity-full#Onboardasub-entity(full)-Retrieveasub-entity)

</Box>

```json withBorder=false
{
  "id": "file_6lbss42ezvoufcb2beo76rvwly",
  "_links": {
    "self": {
      "href": "https://files.sandbox.checkout.com/files/file_6lbss42ezvoufcb2beo76rvwly"
    }
  }
}
```

</tab>
<tab title='422 - Invalid data was sent'>
<Box horizontal='sm'>

In the `error_codes` object, we will let you know what data was invalid.

In the example response, we received `back_required` because the uploaded document was a passport, which requires a front **and** back.

</Box>

```json withBorder=false
{
    "request_id": "0HM72UMS6HIO5:00000001",
    "error_type": "invalid_request",
    "error_codes": [
        "back_required"
    ]
}
```

<Box horizontal='sm'>
<Accordion isBoldTitle={true} title='Possible error codes'>

<Table>
<TableHead headers={['Name','Description']}></TableHead>
<TableBody>
<TableRow>
<TableCell>

`<field_name>_invalid`
For example, `purpose_invalid`.

</TableCell>
<TableCell>

The specified field does not match the validation policies for minimum or maximum length, or formatting.

The only two exceptions to this are `sub_entity_invalid` and `existing_sub_entity_type_invalid`, which have different causes. See the two rows directly above.

</TableCell>
</TableRow>
<TableRow>
<TableCell>

`<field_name>_required`

For example, `legal_name_required` or `representatives_0_first_name_required`.

</TableCell>
<TableCell>

The specified field is mandatory but was not provided in the request. For example, the field is `null`, contains empty strings, or contains white spaces.

</TableCell>
</TableRow>
<TableRow>
<TableCell>

`<side>_file_size_invalid`
For example, `back_file_size_invalid`.

</TableCell>
<TableCell>

Files must be less than 4Mb in size.

</TableCell>
</TableRow>
<TableRow>
<TableCell>

`<side>_file_content_type_invalid`
For example, `front_file_contents_invalid`.

</TableCell>
<TableCell>

Files must be of type PNG or JPG/JPEG.

</TableCell>
</TableRow>
<TableRow>
<TableCell>

`<side>_file_contents_type_invalid`
For example, `back_file_contents_invalid`.

</TableCell>
<TableCell>

The contents of the file are not valid. For example, the extension is PNG, but the contents aren't.

</TableCell>
</TableRow>
<TableRow>
<TableCell>

`back_not_required`

</TableCell>
<TableCell>

A back file is not required for the type of document in your request. This currently only applies to the type `passport`.

</TableCell>
</TableRow>
</TableBody>
</Table>

</Accordion>
</Box>
</tab>
</tabs>

---

## Where to next?

<ContentPanel variant="heading" title="Update a sub-entity" imgSrc="/NAS/Marketplaces/onboard-sub-entities.svg">

Use the file `id` returned by the Files API to attach identity documentation required for `full` due diligence.

<ButtonLinkSecondary href="/marketplaces/about-onboarding/onboard-a-sub-entity-full">Full account type</ButtonLinkSecondary>

</ContentPanel>