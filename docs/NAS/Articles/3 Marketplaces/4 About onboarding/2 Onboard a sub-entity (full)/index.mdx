...
This process should be followed for those on the full account type for our marketplaces solution. We have another page for [lite sub-entity onboarding.](/marketplaces/about-onboarding/onboard-a-sub-entity-lite)
...
<Box horizontal='sm'>
<Accordion isBoldTitle={true} title={`Why haven't my capabilities been enabled?`}>
...
Use our [files API](/marketplaces/about-onboarding/upload-a-file) to upload identity documentation. When you upload a file, the API returns an `id` that represents that file, which you can use to attach identity documentation when creating or updating your sub-entity. 
...
Once we have conducted our due diligence checks, we will inform you of the outcome via [webhook notification](/workflows/webhook-event-types). You should expect to receive the following event types: 
...
<ButtonLinkSecondary href="/marketplaces/accept-a-payment">Learn more</ButtonLinkSecondary>
...
<ContentPanel variant="heading" title="Status types" imgSrc="/NAS/Marketplaces/onboard-sub-entities.svg">

Find out more about status types during the onboarding process.

<ButtonLinkSecondary href="/marketplaces/about-onboarding#status-types">Find out more</ButtonLinkSecondary>
