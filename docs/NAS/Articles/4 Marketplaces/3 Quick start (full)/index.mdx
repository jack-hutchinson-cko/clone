...
You will also notice there is a `status` field set to `requirements_due`. This is because we still require more information before we can conduct due diligence checks – in this case, it is the identification document. Read more [about statuses](/marketplaces/onboarding#Sub-entity_status).
...
Due diligence checks have not yet been conducted. To trigger checks we must provide the outstanding requirements detailed in the previous step, so let’s do that next.
