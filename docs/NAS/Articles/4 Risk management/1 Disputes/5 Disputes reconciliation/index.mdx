...
To generate a disputes report, and view any you've previously created, log in to the Dashboard and go to _Reports_. Click _New report_ to create a new disputes report.

These reports list all of the disputes that occurred during the selected period, with [various columns providing more information about each event](/dashboard/generate-reports#Disputes).
...
**Dashboard**. Log in to the Dashboard and go to Settlements. Click on a settlement from the list to open the settlement details page. In the ‘Transactions breakdown’ section, click Download CSV to download a report of financial actions by payout ID. This will give you a breakdown of transactions for that particular settlement. 
