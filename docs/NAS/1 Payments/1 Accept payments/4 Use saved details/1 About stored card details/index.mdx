...
See the sections on  [cardholder-initiated transactions](https://docs.checkout.com/four/payments/accept-payments/use-saved-details/about-stored-card-details#Aboutstoredcarddetails-cits) and  [merchant-initiated transactions](https://docs.checkout.com/four/payments/accept-payments/use-saved-details/about-stored-card-details#Aboutstoredcarddetails-mits) further down for more detail.
...
- For both [scheduled](https://docs.checkout.com/four/payments/accept-payments/use-saved-details/about-stored-card-details#Aboutstoredcarddetails-scheduled) and [unscheduled](https://docs.checkout.com/four/payments/accept-payments/use-saved-details/about-stored-card-details#Aboutstoredcarddetails-unscheduled) MITs, you must include the `merchant_initiated: true` flag to mark the payment as merchant-initiated, and reference either the [original cardholder-initiated payment or the previous payment](https://docs.checkout.com/four/payments/accept-payments/use-saved-details/about-stored-card-details#Aboutstoredcarddetails-previouspaymentid) in the series of transactions using the `previous_payment_id` attribute.

- The initial cardholder-initiated transaction of a scheduled recurring series should **no**t be marked as `"Recurring"` (unless it is a Visa card, in which case it **must** be marked as `"Recurring"`) and it should have `merchant_initiated` set to `false`. All subsequent payments in the series should include `payment_type`: `"Recurring"`, `merchant_initiated: true` and the `previous_payment_id`.
...
Merchant-initiated transactions (MITs) are payments you trigger, where the cardholder has previously consented to you carrying out such payments. These may be [scheduled](https://docs.checkout.com/four/payments/accept-payments/use-saved-details/about-stored-card-details#Aboutstoredcarddetails-scheduled) (such as recurring payments and installments) or [unscheduled](https://docs.checkout.com/four/payments/accept-payments/use-saved-details/about-stored-card-details#Aboutstoredcarddetails-unscheduled) (like account top-ups triggered by balance thresholds and no-show charges).
...
If you're [adding a Visa card for later scheduled billing without taking a payment](https://docs.checkout.com/four/risk-management/card-verification), you need to include `payment_type: "Recurring"` in your request.
...
<Table>
<TableHead headers={['Series with card verification', 'Standart payment series']} />
<TableBody>
<TableRow>
<TableCell>
