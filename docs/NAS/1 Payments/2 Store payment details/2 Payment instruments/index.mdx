...
- Create a [card payment instrument](/payments/store-payment-details/payment-instruments#Paymentinstruments-card).
- Create a [bank account payment instrument](/payments/store-payment-details/payment-instruments#Paymentinstruments-bankaccount).
...
For the full API specification, see the [API reference](https://api-reference.checkout.com/preview/crusoe/#tag/Payments).
...
Before creating a bank account payment instrument, use the [get bank account field formatting endpoint](https://api-reference.checkout.com/preview/crusoe/#tag/Instruments/paths/~1validation~1bank-accounts~1{country}~1{currency}/get) to check which fields are required for the country and currency of the account.
